...
- (void)setItem:(YYPhotoGroupItem *)item {
    ...
    @weakify(self);
    [_imageView setImageWithURL:[NSURL URLWithString:item.largeImageURLStr] placeholder:item.thumbImage options:kNilOptions progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        @strongify(self);
        if (!self) return;
...
- (instancetype)initWithGroupItems:(NSArray *)groupItems {
    ...
    _pager.currentPageIndicatorTintColor = [UIColor orangeColor];
    _pager.pageIndicatorTintColor = [[UIColor blackColor] colorWithAlphaComponent:0.5];
...
- (void)presentFromImageView:(UIView *)fromView
                 toContainer:(UIView *)toContainer
                  ...
                  completion:(void (^)(void))completion vc:(UIViewController *)sourceVc {
    ...
    if (_blurEffectBackground) {
        _blurBackground.image = [_snapshorImageHideFromView imageByBlurLight]; //Same to UIBlurEffectStyleDark
    } else {
    ...
    BOOL haveAlertTap = [[NSUserDefaults standardUserDefaults] boolForKey:@"haveAlertTap"];
    if (!haveAlertTap) {
        SHOW_ALERT(@"长按可保存图片");
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"haveAlertTap"];
    }
    ...
    if (!item.thumbClippedToTop) {
        NSString *imageKey = [[YYWebImageManager sharedManager] cacheKeyForURL:[NSURL URLWithString:item.largeImageURLStr]];
...
- (void)longPress {
    ...
    _index ++;
    if (_index == 1) {
        [self share:_sourceVc shareImage:tile.imageView.image];
    }
}

- (void)share:(UIViewController *)vc shareImage:(UIImage *)image {
    if (!image) {
        return;
    }

    NSString *textToShare = @"下载私人社区app，去我的主页看看吧";
    UIImage *imageToShare = image;
    NSURL *urlToShare = [NSURL URLWithString:@"https://itunes.apple.com/cn/app/id1127768538"];
    NSArray *activityItems = @[urlToShare,textToShare,imageToShare];
    
    customActivity * custom = [[customActivity alloc] initWithTitie:[SQUser sharedUser].username withActivityImage:image withUrl:urlToShare withType:@"customActivity" withShareContext:activityItems];
    ...
    if([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0){
        UIActivityViewControllerCompletionWithItemsHandler myBlock = ^(NSString *activityType,BOOL completed,NSArray *returnedItems,NSError *activityError) {
            if (completed){
                _index = 0;
                [LCProgressHUD showSuccess:@"操作完成"];
            } else {
                _index = 0;
            }
        };
    ...
    [[AppDelegate appDelegate].window.rootViewController presentViewController:activityVC animated:YES completion:nil];
}
