...
- (void)viewDidLoad {
    ...
    [AppDelegate appDelegate].window.backgroundColor = [UIColor blackColor];
}

- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    [self.navigationController.navigationBar setValue:DefaultNavbarTintColor forKeyPath:@"barTintColor"];
    [self.navigationController.navigationBar setTintColor:DefaultNavTintColor];
    [self.navigationController.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName:DefaultNavTitleColor}];
}
...
- (void)setIsShowDelBtn:(BOOL)isShowDelBtn {
    ...
    if (_isShowDelBtn) {
        ...
        UIButton *btn = [[UIButton alloc] initWithFrame:FRAME(0, 0, 30, 60)];
        [btn setImage:[UIImage imageNamed:@"delBtn"] forState:UIControlStateNormal];
...
//    [self.deleteAssets removeObjectForKey:currentPage];
    ...
    if (_currentPage == -1) {
        _currentPage = 0;
        [self.photos removeObjectAtIndex:_currentPage];
        _currentPage --;
        [self reloadData];
    } else {
        [self.photos removeObjectAtIndex:_currentPage];
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:PICKER_REFRESH_DONE object:nil userInfo:@{@"assets":self.doneAssets}];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"RefreshPhotos" object:nil userInfo:nil];
...
- (void)setPageLabelPage:(NSInteger)page{
    ...
    self.title = [NSString stringWithFormat:@"%ld / %ld",temp, self.photos.count];
}
