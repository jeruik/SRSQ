...
- (void)showPickerVc:(UIViewController *)vc{
    ...
    if (weakVc != nil) {
        [weakVc presentViewController:self animated:YES completion:^{
            if (weakSelf.completedBlock) {
                weakSelf.completedBlock();
            }
        }];
    }
}
...
+ (UIImage *)getImageWithImageObj:(id)imageObj{
    ...
    if ([imageObj isKindOfClass:[UIImage class]]) {
        ...
        MLSelectPhotoAssets *assets = (MLSelectPhotoAssets *)imageObj;
        return [assets originImage];
    }
    return image;
}
@end
